...
<body>
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12 col-md-6">
                                                <div class=" form-group mb-0 text-center">
                                                    ...
                                                    $stmtBaru->execute([
                                                    ':post_id' => $row['id'],
                                                    ':post_judul' => $row['title'], // Adjust as per 
                                                    ':post_desk' => $row['description'], // Adjust as per 
                                                    ':post_publish' => $dateconv, // Adjust as per 
                                                    ':post_datex' => NULL, // Adjust as per 
                                                    ':post_img' => $row['img'], // Adjust as per 
                                                    ':post_see' => $row['total'], // Adjust as per 
                                                    ':ca_id' => str_pad($row['category'], 3, '0', STR_PAD_LEFT), // Adjust as per 
                                                    ':_active' => $row['status'],
                                                    ':_cre' => str_repeat('1', 18),
                                                    ':_cre_date' => date('Y-m-d'),
                                                    ':_chg' => str_repeat('1', 18),
                                                    ...
                                                    function PPID_Post($oldDb, $newDb) {
                                                        ...
                                                        $categories = array(
                                                            ...
                                                            array('<p style="text-align: justify; "><b><span style="font-size:16.0pt;line-height:107%;
                                                            font-family:&quot;Bookman Old Style&quot;,serif;mso-fareast-font-family:Calibri;
                                                            mso-fareast-theme-font:minor-latin;mso-bidi-font-family:Arial;mso-ansi-language:
                                                            EN-US;mso-fareast-language:EN-US;mso-bidi-language:AR-SA">Visi Misi PPID
                                                            Pelaksana</span></b><br></p>', '<p style="text-align: justify; ">Visi
    ...
    sosialisasi keterbukaan Informasi Publik</span></li></ol>', '','','','006'),
                                                            array('Struktur Organisasi PPID Pelaksana', 'desk', '','','','006'),
                                                            ...
                                                            array('Dokumen', 'Dokumen Perencanaan dan Pelaporan', '','','','004'),
                                                            array('Dokumen', 'Dokumen Standar Pelayanan', '','','','004'),
                                                        );
                                                        ...
                                                        try {
                                                            foreach ($categories as $row) {
                                                                ...
                                                                // Jika ID sudah ada, cari ID yang belum terpakai
                                                                while ($count > 0) {
                                                                    $convert++;
                                                                    $checkStmt->execute([':post_id' => $convert]);
                                                                    $count = $checkStmt->fetchColumn();
                                                                ...
                                                                // Lakukan insert dengan ID yang sudah diubah jika diperlukan
                                                                $stmtBaru->execute([
                                                                    ':post_id' => $convert,
                                                                    ':post_judul' => $row[0], 
                                                                    ':post_desk' => $row[1], 
                                                    ...
                                                    $stm2 = $oldDb->prepare("SELECT tb_events.*, tb_counter.idnews,
                                                    tb_counter.total FROM `tb_counter`
                                                    ...
                                                    $stmtBaru = $newDb->prepare("INSERT INTO pub_post (post_id,
                                                    post_judul, post_desk, post_publish, post_datex,
                                                    post_img, post_see, ca_id, _active, _cre, _cre_date, _chg,
                                                    _chg_date)
                                                    VALUES (:post_id, :post_judul, :post_desk, :post_publish,
                                                    :post_datex, :post_img, :post_see, :ca_id, :_active,
                                                    :_cre, :_cre_date, :_chg, :_chg_date)");
                                                    ...
                                                    $dateObj = DateTime::createFromFormat('l d F Y - H:i',
                                                    $datetimestr);
                                                    if ($dateObj) {
                                                    ...
                                                    ':post_id' => $row['id'],
                                                    ':post_judul' => $row['title'], // Adjust as per 
                                                    ':post_desk' => $row['description'], // Adjust as per your data
                                                    ':post_publish' => $formatdatestr, // Adjust as per 
                                                    ':post_datex' => $dateconv, // Adjust as per 
                                                    ':post_img' => $row['img'], // Adjust as per 
                                                    ':post_see' => $row['total'], // Adjust as per 
                                                    ':ca_id' => '002' , // Adjust as per 
                                                    ':_active' =>1,
                                                    ':_cre' => str_repeat('1', 18),
                                                    ':_cre_date' => date('Y-m-d'),
                                                    ':_chg' => str_repeat('1', 18),
                                                    ...
                                                    ':post_judul' => $row['title'], // Adjust as per 
                                                    ':post_desk' => $row['description'], // Adjust as per your data
                                                    ':post_publish' => $row['date'], // Adjust as per 
                                                    ':post_datex' => $row['datex'], // Adjust as per 
                                                    ':post_img' => '' , // Adjust as per 
                                                    ':post_see' => '', // Adjust as per 
                                                    ':ca_id' => '003', // Adjust as per 
                                                    ':_active' =>$row['status'],
                                                    ':_cre' => str_repeat('1', 18),
                                                    ':_cre_date' => date('Y-m-d'),
                                                    ':_chg' => str_repeat('1', 18),
                                                    ...
                                                    ':post_judul' => $row['title'], // Adjust as per 
                                                    ':post_desk' => $row['description'], // Adjust as per your data
                                                    ':post_publish' => $date_pub, // Adjust as per 
                                                    ':post_datex' => NULL, // Adjust as per 
                                                    ':post_img' => '' , // Adjust as per 
                                                    ':post_see' => '', // Adjust as per 
                                                    ':ca_id' => '004', // Adjust as per 
                                                    ':_active' =>1,
                                                    ':_cre' => str_repeat('1', 18),
                                                    ':_cre_date' => date('Y-m-d'),
                                                    ':_chg' => str_repeat('1', 18),
                                                    ...
                                                    $stmtBaru->execute([
                                                    ':post_id' => $row['id'],
                                                    ':post_judul' => $row['name'], // Adjust as per 
                                                    ':post_desk' => $row['description'], // Adjust as per your data
                                                    ':post_publish' => $date_pub, // Adjust as per 
                                                    ':post_datex' => NULL, // Adjust as per 
                                                    ':post_img' => '' , // Adjust as per 
                                                    ':post_see' => '', // Adjust as per 
                                                    ':ca_id' => '005', // Adjust as per 
                                                    ':_active' =>$row['status'],
                                                    ':_cre' => str_repeat('1', 18),
                                                    ':_cre_date' => date('Y-m-d'),
                                                    ':_chg' => str_repeat('1', 18),
                                                    ...
                                                    // Insert data kategori
                                                    $categories = array(
                                                    array('001', 'Form Berita', '_page-post', '1', '555', '2023-03-20',
                                                    '555', '2023-05-02'),
                                                    array('002', 'Form Agenda', '_page-agnd', '1', '555', '2023-03-20',
                                                    '555', '2023-05-08'),
                                                    array('003', 'Form Link', '_page-link', '0', '555', '2023-03-20',
                                                    '555', '2024-04-04'),
                                                    array('004', 'Form Pengumuman', '_page-publik', '1', '555',
                                                    '2023-05-09', '555', '2023-05-16')
                                                    );

                                                    $stmtBaru = $newDb->prepare("INSERT INTO `set_form` (`fm_id`,
                                                    `fm_name`, `fm_file`, `_active`, `_cre`, `_cre_date`,
                                                    `_chg`, `_chg_date`)
                                                    VALUES (:fm_id, :fm_name, :fm_file, :_active, :_cre, :_cre_date,
                                                    :_chg, :_chg_date)");
                                                    ...
                                                    if (isset($_SESSION['oldDbDetails']) &&
                                                    isset($_SESSION['newDbDetails'])) {
                                                    $oldDbDetails = $_SESSION['oldDbDetails'];
                                                    $newDbDetails = $_SESSION['newDbDetails'];
                                                    // Buat koneksi ke database lama
                                                    $oldDb = new
                                                    PDO("mysql:host=localhost;dbname={$oldDbDetails['name']}",
                                                    $oldDbDetails['user'], $oldDbDetails['pass']);
                                                    $oldDb->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                                                    // Buat koneksi ke database baru
                                                    $newDb = new
                                                    PDO("mysql:host=localhost;dbname={$newDbDetails['name']}",
                                                    $newDbDetails['user'], $newDbDetails['pass']);
                                                    $newDb->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                                                    // Panggil fungsi migrasi untuk setiap tabel yang ingin Anda
                                                    // pindahkan
                                                    // setMenu($oldDb, $newDb);
                                                    ...
                                                    echo '<div class="alert alert-danger" role="alert">Database
                                                        connections not found.</div>';
                                                    }
                                                    }
